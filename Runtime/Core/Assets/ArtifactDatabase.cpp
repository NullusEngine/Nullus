...
constexpr const char* kArtifactDatabaseVersion = "3";
    ...
    const ArtifactRecordStatus status)
{
    ...
    m_dependenciesBySourceTarget[MakeKey(manifest.sourceAssetId, {}, manifest.targetPlatform)] = manifest.dependencies;
...
std::optional<ArtifactManifest> ArtifactDatabase::BuildManifestForSource(const AssetId sourceAssetId) const
{
    ...
    if (records.empty())
        return std::nullopt;
    return BuildManifestForSource(sourceAssetId, records.front()->targetPlatform);
}
    ...
    const std::string& targetPlatform) const
{
    ...
    const auto* first = targetRecords.front();
    ArtifactManifest manifest;
    manifest.sourceAssetId = sourceAssetId;
    manifest.importerId = first->importerId;
    manifest.importerVersion = first->importerVersion;
    manifest.targetPlatform = first->targetPlatform;
    manifest.primarySubAssetKey = first->primarySubAssetKey;
...
bool ArtifactDatabase::Save(const std::filesystem::path& path) const
{
    ...
    if (!OpenEnvironment(
            ...
            environment,
            &errorMessage,
            EstimateSaveMapSize(m_records, m_dependenciesBySourceTarget)))
    {
...
bool ArtifactDatabase::Load(const std::filesystem::path& path)
{
    ...
    MDB_val key {};
    MDB_val value {};
    auto cursorResult = mdb_cursor_get(recordsCursor, &key, &value, MDB_FIRST);
    for (;

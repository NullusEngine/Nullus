...
**Independent Test**: A scene containing LOD groups and HLOD clusters verifies that per-view LOD masks choose exactly one active representation per group, HLOD proxy visibility hides child primitives when selected, and fade metadata remains reserved without double-submitting cross-fade representations in this slice.
...
3. **Given** a cluster is near the transition threshold, **When** hysteresis settings apply, **Then** the result is stable across small camera movements; fade/cross-fade metadata is preserved for a later rendering slice but does not currently submit multiple representations.
...
- **FR-011**: Per-object LOD selection MUST use a documented screen-relative metric, camera parameters, LOD bias, and hysteresis; optional fade state is reserved metadata and MUST NOT be claimed as rendered cross-fade until a later slice implements dual-representation submission.
...
- **LODGroupRecord**: Per-object LOD thresholds, reserved fade metadata, and member primitive handles.

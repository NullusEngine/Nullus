...
namespace NLS::Render::Backend
{
	namespace
	{
		class DX12Fence final : public NLS::Render::RHI::RHIFence
		{
		public:
			explicit DX12Fence(std::shared_ptr<NLS::Render::RHI::RHIFence> innerFence)
		...
		class DX12Semaphore final : public NLS::Render::RHI::RHISemaphore
		{
		public:
			explicit DX12Semaphore(std::shared_ptr<NLS::Render::RHI::RHISemaphore> innerSemaphore)
		...
		class DX12CommandBuffer final : public NLS::Render::RHI::RHICommandBuffer
		{
		public:
			explicit DX12CommandBuffer(std::shared_ptr<NLS::Render::RHI::RHICommandBuffer> innerCommandBuffer)
		...
		class DX12CommandPool final : public NLS::Render::RHI::RHICommandPool
		{
		public:
			explicit DX12CommandPool(std::shared_ptr<NLS::Render::RHI::RHICommandPool> innerCommandPool)
			...
			{
				return std::make_shared<DX12CommandBuffer>(m_innerCommandPool->CreateCommandBuffer(std::move(debugName)));
			}
		...
		class DX12Swapchain final : public NLS::Render::RHI::RHISwapchain
		{
		public:
			explicit DX12Swapchain(std::shared_ptr<NLS::Render::RHI::RHISwapchain> innerSwapchain)
			...
			{
				const auto dx12Semaphore = std::dynamic_pointer_cast<DX12Semaphore>(signalSemaphore);
				const auto dx12Fence = std::dynamic_pointer_cast<DX12Fence>(signalFence);
				return m_innerSwapchain->AcquireNextImage(
					dx12Semaphore != nullptr ? dx12Semaphore->GetInnerSemaphore() : signalSemaphore,
					dx12Fence != nullptr ? dx12Fence->GetInnerFence() : signalFence);
			}
		...
		class DX12Queue final : public NLS::Render::RHI::RHIQueue
		{
		public:
			explicit DX12Queue(std::shared_ptr<NLS::Render::RHI::RHIQueue> innerQueue)
			...
			void Submit(const NLS::Render::RHI::RHISubmitDesc& submitDesc) override
			{
				...
				{
					if (const auto dx12CommandBuffer = std::dynamic_pointer_cast<DX12CommandBuffer>(commandBuffer))
						innerDesc.commandBuffers.push_back(dx12CommandBuffer->GetInnerCommandBuffer());
					else
				...
				{
					if (const auto dx12Semaphore = std::dynamic_pointer_cast<DX12Semaphore>(semaphore))
						innerDesc.waitSemaphores.push_back(dx12Semaphore->GetInnerSemaphore());
					else
				...
				{
					if (const auto dx12Semaphore = std::dynamic_pointer_cast<DX12Semaphore>(semaphore))
						innerDesc.signalSemaphores.push_back(dx12Semaphore->GetInnerSemaphore());
					else
						innerDesc.signalSemaphores.push_back(semaphore);
				}
				if (const auto dx12Fence = std::dynamic_pointer_cast<DX12Fence>(submitDesc.signalFence))
					innerDesc.signalFence = dx12Fence->GetInnerFence();
				else
			...
			void Present(const NLS::Render::RHI::RHIPresentDesc& presentDesc) override
			{
				...
				if (const auto dx12Swapchain = std::dynamic_pointer_cast<DX12Swapchain>(presentDesc.swapchain))
					innerDesc.swapchain = dx12Swapchain->GetInnerSwapchain();
				else
		...
		class DX12Adapter final : public NLS::Render::RHI::RHIAdapter
		{
		public:
			explicit DX12Adapter(NLS::Render::RHI::IRenderDevice& renderDevice)
				: m_vendor(renderDevice.GetVendor())
				, m_hardware(renderDevice.GetHardware())
			{
			}

			std::string_view GetDebugName() const override { return "DX12Adapter"; }
		...
		class DX12ExplicitDevice final : public NLS::Render::RHI::RHIDevice
		{
		public:
			explicit DX12ExplicitDevice(NLS::Render::RHI::IRenderDevice& renderDevice)
				: m_nativeInfo(renderDevice.GetNativeDeviceInfo())
				, m_capabilities(renderDevice.GetCapabilities())
				, m_adapter(std::make_shared<DX12Adapter>(renderDevice))
				, m_innerDevice(NLS::Render::RHI::CreateCompatibilityExplicitDevice(renderDevice))
			{
			}

			std::string_view GetDebugName() const override { return "DX12ExplicitDevice"; }
			...
			std::shared_ptr<NLS::Render::RHI::RHIQueue> GetQueue(NLS::Render::RHI::QueueType queueType) override
			{
				...
				if (m_queues[queueIndex] == nullptr)
					m_queues[queueIndex] = std::make_shared<DX12Queue>(m_innerDevice->GetQueue(queueType));
			...
			{
				return std::make_shared<DX12Swapchain>(m_innerDevice->CreateSwapchain(desc));
			}
			...
			{
				return std::make_shared<DX12CommandPool>(m_innerDevice->CreateCommandPool(queueType, std::move(debugName)));
			}
			std::shared_ptr<NLS::Render::RHI::RHIFence> CreateFence(std::string debugName) override
			{
				return std::make_shared<DX12Fence>(m_innerDevice->CreateFence(std::move(debugName)));
			}
			std::shared_ptr<NLS::Render::RHI::RHISemaphore> CreateSemaphore(std::string debugName) override
			{
				return std::make_shared<DX12Semaphore>(m_innerDevice->CreateSemaphore(std::move(debugName)));
			}
	...
	std::shared_ptr<NLS::Render::RHI::RHIDevice> CreateDX12ExplicitDevice(NLS::Render::RHI::IRenderDevice& renderDevice)
	{
		return std::make_shared<DX12ExplicitDevice>(renderDevice);
	}
}

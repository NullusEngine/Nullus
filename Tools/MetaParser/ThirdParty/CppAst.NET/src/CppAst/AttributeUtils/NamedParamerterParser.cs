    ...
    public class NamedParameterParser
    {
        ...
        public class NamedParameterGrammer : Irony.Parsing.Grammar
        {
                ...
                base(true)
            {
                ...
                NonTerminal CALL = new NonTerminal(TerminalNames.Call);
                ...
                CALL.Rule = CLASS_NAME | CLASS_NAME + LEFT_BRACKET + ARGS + RIGHT_BRACKET;
                LOOP_PAIR.Rule = MakeStarRule(COMMA + ASSIGNMENT);
                NAMED_ARGUMENTS.Rule = MakePlusRule(NAMED_ARGUMENTS, ToTerm(","), CALL);
        ...
        public static bool ParseNamedParameters(string content, Dictionary<string, object> outNamedParameterDic, out string errorMessage)
        {
            ...
            if (!ast.HasErrors())
            {
                foreach (var callNode in ast.Root.ChildNodes)
                {
                    ParseCall(callNode, outNamedParameterDic);
                }
        ...
        private static void ParseCall(ParseTreeNode node, Dictionary<string, object> outDic)
        {
            // 第一部分一定是 CLASS_NAME
            string typeName = ParseExpressionValue(node.ChildNodes[0]).ToString();
            ...
            if (node.ChildNodes.Count > 1)
            {
                var argsNode = node.ChildNodes[2]; // LEFT_BRACKET, ARGS, RIGHT_BRACKET
                foreach (var child in argsNode.ChildNodes)
                {
                    args.Add(ParseExpressionValue(child));
                }
            }

            outDic[typeName] = args;
        }

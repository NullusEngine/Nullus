...
- Use documented screen-relative size, LOD bias, hysteresis, and optional fade state.
- Keep selection per view.
- Allow explicit editor/runtime forced LOD.
- Record selected LOD and fade state in `VisibilityFrameResult`.

**Forbidden behavior**:

- Do not rebuild cached draw commands for transform-only LOD evaluation.
- Do not submit multiple LODs except inside an enabled fade transition.

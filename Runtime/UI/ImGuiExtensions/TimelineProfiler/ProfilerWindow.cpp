	...
	if (ImGui::ItemAdd(timelineRect, timelineID))
	{
		...
		*/
		...
		if (ImGui::IsWindowFocused())
		{
			...
			if (mouseWheel != 0.0f && ImGui::IsMouseHoveringRect(timelineRect.Min, timelineRect.Max))
			{
				// Logarithmic scale
				float newScale = TimelineProfilerDetail::ComputeTimelineWheelZoomScale(context.TimelineScale, mouseWheel);
...
bool PrepareProfilerHUD()
{
	...
	return NLS::UI::Icons::EnsureFontAwesomeIconFontLoaded();
}

void DrawProfilerHUD()
{
	if (!PrepareProfilerHUD())
		return;

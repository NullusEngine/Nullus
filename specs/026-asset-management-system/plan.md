...
5. **Shader artifact pipeline**: Imports `.hlsl` source assets through the same `AssetDatabaseFacade` path as model and prefab sources, writes `.shader` payloads under GUID-keyed `Library/Artifacts`, records stage/target compile metadata in `ArtifactDB`, and lets runtime shader loading consume committed artifacts without source-path compilation on hot paths.

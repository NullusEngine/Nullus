...
TEST_F(GameLaunchArgsTests, RejectsRuntimeAssetDatabaseWithMixedTargetPlatforms)
{
    const auto root =
        std::filesystem::temp_directory_path() /
    ...
    EXPECT_FALSE(runtimeDatabase.has_value())
        << "Runtime startup must not silently merge editor and runtime target artifacts.";

...
Maths::Quaternion::Quaternion(float p_real)
    : x(0.0f), y(0.0f), z(0.0f), w(p_real)
{
}

Maths::Quaternion::Quaternion(float p_x, float p_y, float p_z, float p_w)
    : x(p_x), y(p_y), z(p_z), w(p_w)
{
}

Maths::Quaternion::Quaternion(const Quaternion& p_other)
...
{
    auto vector = Maths::Vector3::Normalize(p_forward);
    auto vector2 = Maths::Vector3::Normalize(Maths::Vector3::Cross(p_up, vector));
    auto vector3 = Maths::Vector3::Cross(vector, vector2);
    auto m00 = vector2.x;
    auto m01 = vector2.y;
    auto m02 = vector2.z;
    auto m10 = vector3.x;
    ...
    auto quaternion = Maths::Quaternion::Identity;
    if (num8 > 0.f)
    {
...
bool Maths::Quaternion::IsIdentity(const Quaternion& p_target)
{
    return p_target.w == 1.0f && Length(p_target) == 1.0f;
}

bool Maths::Quaternion::IsPure(const Quaternion& p_target)
{
    return p_target.w == 0.0f && (p_target.x != 0.0f || p_target.y != 0.0f || p_target.z != 0.0f);
}

bool Maths::Quaternion::IsNormalized(const Quaternion& p_target)
{
    return abs(Length(p_target) - 1.0f) < 0.0001f;
}

float Maths::Quaternion::DotProduct(const Quaternion& p_left, const Quaternion& p_right)
{
    return p_left.x * p_right.x + p_left.y * p_right.y + p_left.z * p_right.z + p_left.w * p_right.w;
}

Maths::Quaternion Maths::Quaternion::Normalize(const Quaternion& p_target)
{
    return p_target / Length(p_target);
}

float Maths::Quaternion::Length(const Quaternion& p_target)
{
    return sqrtf(LengthSquare(p_target));
}

float Maths::Quaternion::LengthSquare(const Quaternion& p_target)
{
    return p_target.x * p_target.x + p_target.y * p_target.y + p_target.z * p_target.z + p_target.w * p_target.w;
}

float Maths::Quaternion::GetAngle(const Quaternion& p_target)
{
    return 2.0f * acos(p_target.w);
}

Maths::Vector3 Maths::Quaternion::GetRotationAxis(const Quaternion& p_target)
{
    const float S = sqrt(std::max(1.f - (p_target.w * p_target.w), 0.f));

    if (S >= 0.0001f)
    {
        return Vector3(p_target.x / S, p_target.y / S, p_target.z / S);
    }

    return Vector3(1.f, 0.f, 0.f);
}

Maths::Quaternion Maths::Quaternion::Inverse(const Quaternion& p_target)
{
    return Conjugate(p_target) / LengthSquare(p_target);
}

Maths::Quaternion Maths::Quaternion::Conjugate(const Quaternion& p_target)
{
    return {-p_target.x, -p_target.y, -p_target.z, p_target.w};
}

Maths::Quaternion Maths::Quaternion::Square(const Quaternion& p_target)
{
    return p_target * p_target;
}

std::pair<Maths::Vector3, float> Maths::Quaternion::GetAxisAndAngle(const Quaternion& p_target)
{
    return std::pair<Vector3, float>(GetRotationAxis(p_target), GetAngle(p_target));
}

float Maths::Quaternion::AngularDistance(const Quaternion& p_left, const Quaternion& p_right)
{
    float innerProd = (p_left | p_right);
    return acos((2.0f * innerProd * innerProd) - 1.0f);
}

Maths::Quaternion Maths::Quaternion::Lerp(const Quaternion& p_start, const Quaternion& p_end, float p_alpha)
{
    p_alpha = std::clamp(p_alpha, 0.f, 1.f);

    Quaternion q;

    if (Quaternion::DotProduct(p_start, p_end) < 0.f)
    {
...
Maths::Quaternion Maths::Quaternion::Slerp(const Quaternion& p_start, const Quaternion& p_end, float p_alpha)
{
    Quaternion from = p_start;
    Quaternion to = p_end;

    p_alpha = std::clamp(p_alpha, 0.f, 1.f);
    float cosAngle = Quaternion::DotProduct(from, to);
    ...
    if (cosAngle < 0.95f)
    {
        ...
        return Quaternion(from.x * t1 + to.x * t2, from.y * t1 + to.y * t2, from.z * t1 + to.z * t2, from.w * t1 + to.w * t2);
    }
    else
    {
        return Quaternion::Lerp(from, to, p_alpha);
    }
}

Maths::Quaternion Maths::Quaternion::Nlerp(const Quaternion& p_start, const Quaternion& p_end, float p_alpha)
{
    return Normalize(Lerp(p_start, p_end, p_alpha));
}

Maths::Vector3 Maths::Quaternion::RotatePoint(const Vector3& p_point, const Quaternion& p_quaternion)
{
    Vector3 Q(p_quaternion.x, p_quaternion.y, p_quaternion.z);
    Vector3 T = Vector3::Cross(Q, p_point) * 2.0f;

    return p_point + (T * p_quaternion.w) + Vector3::Cross(Q, T);
}

Maths::Vector3 Maths::Quaternion::RotatePoint(const Vector3& p_point, const Quaternion& p_quaternion, const Vector3& p_pivot)
{
    Vector3 toRotate = p_point - p_pivot;
    return RotatePoint(toRotate, p_quaternion);
}

Maths::Vector3 Maths::Quaternion::EulerAngles(const Quaternion& p_target)
{
    ...
    if (p_target == Quaternion{0.5f, 0.5f, -0.5f, 0.5f})
    ...
    return TO_DEGREES(Vector3(roll, pitch, yaw)); // XYZ
}
...
bool Maths::Quaternion::operator==(const Quaternion& p_otherQuat) const
{
...
Maths::Quaternion Maths::Quaternion::operator+(const Quaternion& p_otherQuat) const
{
    return Quaternion(x + p_otherQuat.x, y + p_otherQuat.x, z + p_otherQuat.z, w + p_otherQuat.w);
}

Maths::Quaternion& Maths::Quaternion::operator+=(const Quaternion& p_otherQuat)
{
    x += p_otherQuat.x;
    y += p_otherQuat.x;
    z += p_otherQuat.z;
...
float Maths::Quaternion::operator|(const Quaternion& p_otherQuat) const
{
    return (x * p_otherQuat.x + y * p_otherQuat.x + z * p_otherQuat.z + w * p_otherQuat.w);
}

Maths::Quaternion Maths::Quaternion::operator*(const float p_scale) const
{
...
Maths::Quaternion& Maths::Quaternion::operator*=(const float p_scale)
{
    x *= p_scale;
    y *= p_scale;
    z *= p_scale;
...
{
    Quaternion temp(
        x * p_otherQuat.w + y * p_otherQuat.z - z * p_otherQuat.x + w * p_otherQuat.x,
        -x * p_otherQuat.z + y * p_otherQuat.w + z * p_otherQuat.x + w * p_otherQuat.x,
        x * p_otherQuat.x - y * p_otherQuat.x + z * p_otherQuat.w - w * p_otherQuat.z,
        -x * p_otherQuat.x - y * p_otherQuat.x - z * p_otherQuat.z + w * p_otherQuat.w);
...
Maths::Vector3 Maths::Quaternion::operator*(const Vector3& p_toMultiply) const
{
    ...
    Vector3 result;
    result.x = (1.f - (num5 + num6)) * p_toMultiply.x + (num7 - num12) * p_toMultiply.y + (num8 + num11) * p_toMultiply.z;
    result.y = (num7 + num12) * p_toMultiply.x + (1.f - (num4 + num6)) * p_toMultiply.y + (num9 - num10) * p_toMultiply.z;
    result.z = (num8 - num11) * p_toMultiply.x + (num9 + num10) * p_toMultiply.y + (1.f - (num4 + num5)) * p_toMultiply.z;
    return result;
}

Maths::Matrix3 Maths::Quaternion::operator*(const Matrix3& p_multiply) const
{

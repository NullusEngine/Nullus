...
void Rendering::Resources::Shader::SetUniformVec2(const std::string & p_name, const Maths::Vector2 & p_vec2)
{
	glUniform2f(GetUniformLocation(p_name), p_vec2.x, p_vec2.y);
}

void Rendering::Resources::Shader::SetUniformVec3(const std::string& p_name, const Maths::Vector3& p_vec3)
{
	glUniform3f(GetUniformLocation(p_name), p_vec3.x, p_vec3.y, p_vec3.z);
}

void Rendering::Resources::Shader::SetUniformVec4(const std::string& p_name, const Maths::Vector4& p_vec4)
{
	glUniform4f(GetUniformLocation(p_name), p_vec4.x, p_vec4.y, p_vec4.z, p_vec4.w);
}

void Rendering::Resources::Shader::SetUniformMat4(const std::string& p_name, const Maths::Matrix4& p_mat4)
{
...
Maths::Vector2 Rendering::Resources::Shader::GetUniformVec2(const std::string& p_name)
{
	GLfloat values[2];
...
Maths::Vector3 Rendering::Resources::Shader::GetUniformVec3(const std::string& p_name)
{
	GLfloat values[3];
	glGetUniformfv(id, GetUniformLocation(p_name), values);
...
Maths::Vector4 Rendering::Resources::Shader::GetUniformVec4(const std::string& p_name)
{
	GLfloat values[4];
	glGetUniformfv(id, GetUniformLocation(p_name), values);
...
Maths::Matrix4 Rendering::Resources::Shader::GetUniformMat4(const std::string& p_name)
{
	GLfloat values[16];
	glGetUniformfv(id, GetUniformLocation(p_name), values);
...
void Rendering::Resources::Shader::QueryUniforms()
{
	...
	for (int unif = 0; unif < numActiveUniforms; ++unif)
	{
		...
		if (!IsEngineUBOMember(name))
		{
			...
			case Rendering::Resources::UniformType::UNIFORM_FLOAT_VEC2:	defaultValue = std::make_any<Maths::Vector2>(GetUniformVec2(name));		break;
			case Rendering::Resources::UniformType::UNIFORM_FLOAT_VEC3:	defaultValue = std::make_any<Maths::Vector3>(GetUniformVec3(name));		break;
			case Rendering::Resources::UniformType::UNIFORM_FLOAT_VEC4:	defaultValue = std::make_any<Maths::Vector4>(GetUniformVec4(name));		break;

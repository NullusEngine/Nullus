...
- **FR-089**: `.hlsl` shader source assets MUST import through `AssetDatabaseFacade` into extensionless content-addressed `Library/Artifacts/<first-two-hash-chars>/<hash>` shader payloads and `Library/ArtifactDB` LMDB records, with source/meta/importer/build-target dependencies recorded like model and prefab imports.

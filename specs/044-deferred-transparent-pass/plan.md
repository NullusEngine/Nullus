...
- Material artifact check: `dirt_decal` in `TestProject\Library\Artifacts\907a615f-bfd4-4d8c-b8e9-6dd60ec33ed0\materials\material%3Amaterial%2F21.nmat` has `<blendable>true</blendable>` but no `<surfaceMode>`, while the source glTF material has `"alphaMode": "BLEND"` and `"name": "dirt_decal"`.
